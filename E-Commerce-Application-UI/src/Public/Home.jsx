  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-16">
      ...
      <div className=" grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {(searchResults.length > 0 ? searchResults : filteredProducts).map((product) => (
          <div key={product.Id} className="bg-white shadow-md rounded p-4  hover:border 4px border-orange-300  cursor-pointer " onClick={
            ...
            <img className="peer-img relative overflow-hidden transition-transform transform hover:scale-110 mb-10" width={350} height={150} src={`http://localhost:8080/api/ecav1/image/${product.coverImage}`} alt={product.productBrand} />
      ...
      <div className='flex justify-center'>
        <button className='px-3 py-2 bg-slate-400' onClick={() => handleClick({page}-1)}>Prev</button>
        <button className='px-1 py-2 bg-slate-200' onClick={(e) => handleClick(e.target.value)}>{{page}+1}</button>
        <button className='px-1 py-2 bg-slate-200' onClick={() => handleClick({page}+2)}>{page+2}</button>
        <button className='px-1 py-2 bg-slate-200' onClick={() => handleClick({page}+3)}>{page+3}</button>
        <button className='px-3 py-2 bg-slate-400' onClick={() => handleClick({page}+1)}>Next</button>
        </div>
    </div>
  );
};
